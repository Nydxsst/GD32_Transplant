...
void SPI1_Init(void)
{
	...
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_64;	//定义波特率预分频的值，APB1预分频而来（72M）
...
{
	u8 retry = 0;
	while(SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_TXE) == RESET)  //等待SPI可发送数据
	{
		retry++;
	...
	while(SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_RXNE) == RESET)  //等待SPI可接收数据
	{
		retry++;
		if(retry > 200)return 0;
	}
